...
export default class App extends Component {
  ...
  render() {
    const list1 = this.state.data.slice(0,2).map(function(item, index) { 
    ...
    const list2 = this.state.data.slice(2,4).map(function(item, index) { 
      const { image, name, occupation, description, showThumbnail } = item;
      return <ProfileCard key={'card-' + index}
                   image={image}
                   style={[{flex: 1}]}
      ...
      const list3 = this.state.data.slice(4,6).map(function(item, index) { 
      const { image, name, occupation, description, showThumbnail } = item;
      return <ProfileCard key={'card-' + index}
                   image={image}
                   style={[{flex: 1}]}
